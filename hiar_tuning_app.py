...
def calculate_axis_full(cc, bore, stroke, cr, rpm_limit, valve_in, valve_out, venturi, dur_in, dur_out, afr, std):
...
with st.sidebar:
    ...
    if 'sb_bore' not in st.session_state or st.session_state.get('active_m') != model:
        st.session_state.sb_bore = float(std['bore'])
        st.session_state.sb_cc = (math.pi * (float(std['bore'])**2) * float(std['stroke'])) / 4000
        st.session_state.active_m = model
    ...
    with st.expander("🛠️ Perimeter Dasar & Sync", expanded=True):
        ...
        in_bore = st.number_input(f"Bore (std: {std['bore']})", key="in_bore", value=st.session_state.sb_bore, step=0.1, on_change=sync_cc)
        in_cc = st.number_input(f"CC Motor Real (std: {(math.pi*(std['bore']**2)*std['stroke'])/4000:.1f})", key="in_cc", value=st.session_state.sb_cc, step=0.1, on_change=sync_bore)
    ...
    if expert_on:
        with st.expander("🧪 Expert Tuning", expanded=True):
...
if run_btn:
    ...
    rpms, hps, torques, gsin, gsout = calculate_axis_full(cc_f, bore_f, in_stroke, cr_f, in_rpm, in_v_in, in_v_out, in_venturi, in_dur_in, in_dur_out, in_afr, std)
    ...
    st.session_state.history.append({
        ...
        "v_in": in_v_in, "v_out": in_v_out, "bore": bore_f, "stroke": in_stroke
    })
...
if st.session_state.history:
    ...
    st.success(f"**3. Solusi:** Bubut kubah head ke {vhead_ideal} cc. {'Ganti Klep In minimal ' + str(round(latest['bore']*0.52, 1)) + 'mm.' if latest['gsin'] > 105 else 'Konfigurasi sudah harmonis.'}")
