...
# --- 3. SESSION STATE (FIXED SIDEBAR SYNC) ---
if 'history' not in st.session_state: st.session_state.history = []

# --- 4. CORE ENGINE LOGIC (GRAHAM BELL BASED) ---
def calculate_axis_ultimate(cc, bore, stroke, cr, rpm_limit, valve_in, valve_out, venturi, dur_in, dur_out, afr, std):
...
with st.sidebar:
    ...
    # Initialize State
    if 'sb_bore' not in st.session_state or st.session_state.get('last_m') != model:
        st.session_state.sb_bore = std['bore']
        st.session_state.sb_cc = (math.pi * (std['bore']**2) * std['stroke']) / 4000
        st.session_state.last_m = model
    ...
    with st.expander("🛠️ Perimeter Dasar & Sync", expanded=True):
        ...
        in_bore = st.number_input(f"Bore (std: {std['bore']})", key="in_bore", 
                                 value=st.session_state.sb_bore, step=0.1, on_change=sync_cc)
        in_cc = st.number_input(f"CC Motor Real (std: {(math.pi*(std['bore']**2)*std['stroke'])/4000:.1f})", 
                               key="in_cc", value=st.session_state.sb_cc, step=0.1, on_change=sync_bore)
    ...
    if expert_on:
        with st.expander("🧪 Expert Details", expanded=True):
            ...
            # FIX: Ensure value and step are both floats to avoid StreamlitMixedNumericTypesError
...
# --- 6. EXECUTION ---
if run_btn:
    cc_f = st.session_state.in_cc
    bore_f = st.session_state.in_bore
    cr_f = (cc_f + in_vhead) / in_vhead
    rpms, hps, torques, gsin, gsout = calculate_axis_ultimate(cc_f, bore_f, in_stroke, cr_f, in_rpm, in_v_in, in_v_out, in_venturi, in_dur_in, in_dur_out, in_afr, std)
    ...
    st.session_state.history.append({
        ...
        "gsin": gsin, "gsout": gsout, "v_in": in_v_in, "v_out": in_v_out, "afr": in_afr, "bore": bore_f
    })
...
if st.session_state.history:
    ...
    # FLOWBENCH PARAMETERS
    st.header("🌪️ Axis Expert Flowbench Analysis")
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("Flow In (CFM)", f"{round((latest['v_in'] / 25.4)**2 * math.sqrt(28) * 128, 1)}")
    with c2: st.metric("Flow Out (CFM)", f"{round((latest['v_out'] / 25.4)**2 * math.sqrt(28) * 128, 1)}")
    with c3: st.metric("Mean Gas Speed In", f"{latest['gsin']:.1f} m/s")
    with c4: st.metric("Mean Gas Speed Out", f"{latest['gsout']:.1f} m/s")
    ...
    fig.update_layout(template="plotly_dark", height=550, paper_bgcolor="#000", plot_bgcolor="#000",
                      xaxis=dict(title="RPM"), yaxis=dict(title="Power (HP)"), yaxis2=dict(overlaying="y", side="right", title="Torque (Nm)"))
    st.plotly_chart(fig, use_container_width=True)

    # ANALYSIS
    st.divider()
    st.header("🏁 Axis Expert Analysis & Solutions")
    st.info(f"**1. Analisa Performa:** Kapasitas {latest['CC']}cc pada AFR {latest['afr']}. Gas Speed In: {latest['gsin']:.1f} m/s.")
    ...
    st.success(f"**3. Solusi:** Bubut kubah head ke {vhead_ideal} cc. {'Ganti Klep In minimal ' + str(round(latest['bore']*0.52, 1)) + 'mm.' if latest['gsin'] > 105 else 'Konfigurasi sudah efisien.'}")

st.write("---")
st.error("⚠️ **DISCLAIMER:** Simulator berbasis rumus. Hasil nyata dapat berbeda.")
