...
if 'sb_bore' not in st.session_state: st.session_state.sb_bore = 50.0
if 'sb_cc' not in st.session_state: st.session_state.sb_cc = 115.0

# --- 4. CORE ENGINE LOGIC (DYNO HIGH-END) ---
def calculate_axis_v13(cc, bore, stroke, cr, rpm_limit, valve_in, valve_out, venturi, dur_in, dur_out, afr, std):
    rpms = np.arange(2000, rpm_limit + 100, 100)
    hps, torques = [], []
    
    # Graham Bell: Duration & LSA Impact
    avg_dur = (dur_in + dur_out) / 2
    peak_shift = (avg_dur - 240) * 55 
    adjusted_peak = std['peak_rpm'] + peak_shift
    
    # Efficiency & AFR Impact
...
with st.sidebar:
    ...
    if st.button("🔄 Reset ke Pabrikan"):
        st.session_state.sb_bore = std['bore']
        st.session_state.sb_cc = (math.pi * (std['bore']**2) * std['stroke']) / 4000

    st.divider()
    st.header("2️⃣ ENGINE SIMULATION")
    
    with st.expander("🛠️ Perimeter 1 (Standard & Sync)", expanded=True):
        label_run = st.text_input("Label Run", value=f"Run {len(st.session_state.history)+1}")
        
        # Logic Sync Bore & CC
        def sync_cc():
            st.session_state.sb_cc = (math.pi * (st.session_state.in_bore**2) * std['stroke']) / 4000
        def sync_bore():
            st.session_state.sb_bore = math.sqrt((st.session_state.in_cc * 4000) / (math.pi * std['stroke']))

        in_bore = st.number_input(f"Bore (std: {std['bore']})", key="in_bore", value=st.session_state.sb_bore, on_change=sync_cc)
        in_cc = st.number_input(f"CC Motor Real (std: {(math.pi * (std['bore']**2) * std['stroke']) / 4000:.1f})", key="in_cc", value=st.session_state.sb_cc, on_change=sync_bore)
    ...
    if expert_on:
        ...
        in_stroke, in_valve_in, in_valve_out, in_venturi, in_dur_in, in_dur_out, in_afr, in_fratio = std['stroke'], std['valve_in'], std['valve_out'], std['venturi'], 240, 240, 13.0, std['f_ratio']
...
if run_btn:
    ...
    rpms, hps, torques, gsin, gsout = calculate_axis_v13(cc_f, in_bore, in_stroke, cr_f, in_rpm, in_valve_in, in_valve_out, in_venturi, in_dur_in, in_dur_out, in_afr, std)
    ...
    st.session_state.history.append({
        ...
        "v_in": in_valve_in, "v_out": in_valve_out, "bore": in_bore, "stroke": in_stroke
    })

# --- 7. MAIN PANEL ---
st.title("📟 Hiar Lima Pendawa Tuning")
st.caption("Professional High-End Engine Analysis | Ver 13.0")
...
if st.session_state.history:
    ...
    fig.update_layout(template="plotly_dark", height=600, paper_bgcolor="#000", plot_bgcolor="#000",
                      xaxis=dict(title="Engine Speed (RPM)", gridcolor="#1a1a1a"), 
    ...
    if latest['gsin'] > 105: analisa += "Terdapat indikasi Choke Flow: Ukuran Klep In tidak sanggup melayani volume silinder di RPM tinggi."
    st.info(analisa)

    st.write("**2. Rekomendasi Expert:**")
    vhead_ideal = round(latest['CC'] / (12.5 - 1), 2)
    v_ratio = (latest['v_out'] / latest['v_in']) * 100
    rekomendasi = f"Target Vol Head ideal untuk CR 12.5:1 adalah **{vhead_ideal} cc**. Rasio Klep Out/In saat ini {v_ratio:.1f}% (Ideal: 80-85%)."
    ...
    elif latest['afr'] > 13.5: solusi += "Tambahkan durasi injeksi pada ECU untuk menghindari panas berlebih."
    else: solusi += "Konfigurasi sudah harmonis, fokus pada optimasi pengapian."
    st.success(solusi)

st.write("---")
st.error("⚠️ **DISCLAIMER:** Aplikasi ini adalah simulator berbasis rumus. Hasil nyata bergantung pada kualitas porting, desain knalpot, dan kondisi lingkungan.")
